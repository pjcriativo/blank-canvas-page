...
import { Check, ChevronDown, ChevronLeft, ChevronRight, ChevronUp, Lightbulb, Maximize2, Minus, Plus, Search, Sparkles, X } from "lucide-react";
...
function normalizeSvgFraming(svgEl: SVGSVGElement, pageIndex: number) {
  ...
  const elements = Array.from(svgEl.querySelectorAll<SVGGraphicsElement>("path, rect, circle, ellipse, polygon, line, polyline"));
...
export const ColoringCanvas = forwardRef<HTMLDivElement, Props>(function ColoringCanvas(
  { svg, fills, signatureName, onFill, showSuggestion, onCloseSuggestion, pageIndex, totalPages, pageSvgs = [], pageFills = {}, onSelectPage, onPrev, onNext, onVerifyComplete, onForceComplete },
  ref,
) {
  ...
  const clampPan = (nextZoom: number, nextPan: { x: number; y: number }, container: HTMLElement | null) => {
  ...
  const renderedSvg = useMemo(() => buildRenderableSvg(svg, fills, signatureName), [svg, fills, signatureName]);

  const previewSvg = useMemo(() => buildRenderableSvg(svg, suggestedFills), [svg, suggestedFills]);
  const renderedThumbnailSvgs = useMemo(() => pageSvgs.map((pageSvg, index) => buildRenderableSvg(pageSvg, pageFills[index] ?? {})), [pageFills, pageSvgs]);
  ...
  const missingChecklistOverflow = Math.max(0, missingChecklist.length - missingChecklistVisible.length);
  ...
  useLayoutEffect(() => {
    ...
    Object.entries(fills).forEach(([id, nextFill]) => {
      ...
      if (container) {
        ...
        const rect = container.getBoundingClientRect();
        spawnFloater(
          bbox.left + bbox.width / 2 - rect.left,
          bbox.top + bbox.height / 2 - rect.top,
        );
      }
    });
  ...
  useEffect(() => {
    thumbnailButtonRefs.current[pageIndex]?.scrollIntoView({ block: "nearest", behavior: "smooth" });
  }, [pageIndex]);
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const axis: "x" | "y" = overflowsY && el.clientHeight > el.clientWidth ? "y" : overflowsX ? "x" : "y";
  ...
  useEffect(() => {
    const wrap = wrapRef.current;
    if (!wrap) return;
    const paintFromPoint = (target: EventTarget | null, clientX: number, clientY: number, skipAlreadyPainted = false) => {
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      if (isGesturingRef.current && gesturePointersRef.current.size >= 2 && gestureStartRef.current) {
          ...
          <div className="rounded-2xl border border-border/80 bg-card/95 p-3 shadow-hero backdrop-blur lg:hidden">
            ...
            {mobileGridPagesTotal > 1 && (
              <div className="mt-3 flex items-center justify-center gap-3">
                ...
                <button
                  type="button"
                  onClick={() => setMobileGridPage((p) => Math.min(mobileGridPagesTotal - 1, p + 1))}
          ...
          >
            ...
            */}
            <div className="-mx-3 -mt-3 mb-2 hidden border-b border-border/80 bg-card px-3 py-2.5 lg:block xl:-mx-4 xl:-mt-4 xl:px-4 xl:py-3">
              <p className="font-display text-sm font-extrabold text-foreground xl:text-base">{t("pages")}</p>
              <p className="mt-0.5 text-[11px] font-semibold text-muted-foreground xl:text-xs">{t("chooseOneOf", { total: totalPages })}</p>
            </div>
            {/*
            ...
            */}
            ...
            {thumbnailSvgs.map((thumbSvg, index) => (
              <button
                ...
                ref={(node) => { thumbnailButtonRefs.current[index] = node; }}
                type="button"
                onClick={() => onSelectPage?.(index)}
