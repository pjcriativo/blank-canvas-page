...
import { Bell, CheckCircle2, Heart, Loader2, LogOut, Search, Smartphone, Sparkles, UserCircle2 } from "lucide-react";
...
export function AppHeader() {
  ...
  if (loc.pathname.startsWith("/colorir") || loc.pathname.startsWith("/admin") || loc.pathname === "/") {
    return null;
  }
      ...
      <div className="flex h-14 items-center justify-between gap-3 px-5 sm:h-16 sm:px-8 lg:px-12">
        ...
        <div className="flex items-center gap-0.5 sm:gap-1">
          ...
          <Popover onOpenChange={(open) => open && markNewsAsSeen()}>
            ...
            <PopoverContent align="end" sideOffset={8} className="max-h-[min(78dvh,34rem)] w-[calc(100vw-2rem)] max-w-96 overflow-hidden rounded-[1.75rem] border-border/80 bg-popover/95 p-0 shadow-elevated backdrop-blur-xl sm:w-96">
              ...
              {newStories.length > 0 ? (
                ...
                <div className="px-5 py-8 text-center text-sm text-muted-foreground">{t("noNews")}</div>
              )}
            </PopoverContent>
          </Popover>
          <Link
            ...
            >
              ...
              */}
              {!isStandalone && (
                ...
                >
                  {installing ? (
                    <Loader2 className="h-[18px] w-[18px] animate-spin text-primary" strokeWidth={1.9} />
                  ) : (
