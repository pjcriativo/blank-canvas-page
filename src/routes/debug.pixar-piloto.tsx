    ...
    <div className="min-h-screen bg-background p-6 flex flex-col items-center gap-8">
      <header className="max-w-3xl text-center space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          3 imagens-piloto geradas com IA (Nano Banana Pro). Aprove para eu produzir as 30
          páginas + replicar nas demais histórias.
        </p>
      </header>
      ...
      <footer className="max-w-3xl text-sm text-muted-foreground space-y-2 pt-4 border-t border-foreground/10">
        <p>
          <strong>Próximo passo:</strong> se aprovar o estilo, eu gero as 30 páginas restantes
          da Arca de Noé seguindo exatamente esse padrão e integro no app (capa decorativa +
          páginas pintáveis com regiões detectadas automaticamente).
        </p>
        <p>
          <strong>Importante:</strong> imagens IA (raster) não pintam por região como o SVG
          atual. A pintura passa a ser por "balde de tinta" sobre áreas brancas detectadas —
          mecânica equivalente, visual MUITO superior.
        </p>
      </footer>
    </div>
  );
