...
function resolvePeriod(request: Request) {
  ...
  const now = new Date();
  const start = period === "month" ? new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), 1)) : startOfUtcDay(now);
    ...
    handlers: {
      GET: async ({ request }) => {
        try {
          ...
          const [{ data: sales }, { data: subscriptions }, { data: plans }, { count: usersTotal }] = await Promise.all([
            admin.from("sales").select("id,amount_cents,currency,status,sold_at,customer_email,plans(name)").gte("sold_at", rangeStart).lte("sold_at", rangeEnd).order("sold_at", { ascending: false }).limit(1000),
            admin.from("subscriptions").select("id,status,amount_cents,current_period_end,started_at,canceled_at,created_at,plans(name)").limit(2000),
            admin.from("plans").select("id,name,active,price_cents,billing_interval").order("created_at", { ascending: false }),
            admin.from("profiles").select("id", { count: "exact", head: true }).lte("created_at", rangeEnd),
          ]);

          const paidSales = (sales ?? []).filter((sale: any) => PAID_STATUSES.has(String(sale.status).toLowerCase()));
          const inRange = (startIso: string) => paidSales.filter((sale: any) => sale.sold_at >= startIso && sale.sold_at <= rangeEnd);
          const sum = (items: any[]) => items.reduce((total, sale) => total + Number(sale.amount_cents ?? 0), 0);
          ...
          const planMap = paidSales.reduce<Record<string, { name: string; count: number; revenue: number }>>((acc, sale: any) => {
          ...
          const statusMap = (subscriptions ?? []).reduce<Record<string, number>>((acc, sub: any) => {
            const status = String(sub.status ?? "pending").toLowerCase();
            acc[status] = (acc[status] ?? 0) + 1;
            return acc;
          }, {});

          const mrr = activeSubscriptions.reduce((total: number, sub: any) => total + Number(sub.amount_cents ?? 0), 0);
          const averageTicket = paidSales.length ? Math.round(sum(paidSales) / paidSales.length) : 0;
          ...
          return json({
            ...
            topPlans: Object.values(planMap).sort((a, b) => b.revenue - a.revenue).slice(0, 8),
            dailyRevenue: buildDailyRevenue(start, end, paidSales),
            subscriptionStatus: Object.entries(statusMap).map(([status, count]) => ({ status, count })),
