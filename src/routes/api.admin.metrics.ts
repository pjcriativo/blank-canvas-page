    ...
    handlers: {
      GET: async ({ request }) => {
        try {
          ...
          const [{ data: sales }, { data: subscriptions }, { data: plans }, { data: profiles }, { count: usersTotal }] = await Promise.all([
            admin.from("sales").select("amount_cents,status,sold_at,plans(name)").gte("sold_at", rangeStart).order("sold_at", { ascending: true }).limit(1000),
            admin.from("subscriptions").select("status,current_period_end,amount_cents,plans(name)").limit(1000),
            admin.from("plans").select("id,name,active,price_cents,billing_interval,native_language").order("created_at", { ascending: false }),
            admin.from("profiles").select("id,user_id,display_name,purchase_email,created_at").order("created_at", { ascending: false }).limit(20),
            admin.from("profiles").select("id", { count: "exact", head: true }),
          ]);
          const paidSales = (sales ?? []).filter((sale: any) => PAID_STATUSES.has(String(sale.status).toLowerCase()));
          const revenuePeriod = paidSales.reduce((sum: number, sale: any) => sum + Number(sale.amount_cents ?? 0), 0);
          const planSales = paidSales.reduce<Record<string, { name: string; count: number; revenue: number }>>((acc, sale: any) => {
          ...
          const subscriptionStatus = (subscriptions ?? []).reduce<Record<string, number>>((acc, sub: any) => {
            const status = String(sub.status ?? "pending").toLowerCase();
            acc[status] = (acc[status] ?? 0) + 1;
            return acc;
          }, {});
          const activeSubscriptions = (subscriptions ?? []).filter((sub: any) => String(sub.status).toLowerCase() === "active");
          return json({
            period,
            periodLabel: label,
            ...
            mrr: activeSubscriptions.reduce((sum: number, sub: any) => sum + Number(sub.amount_cents ?? 0), 0),
            ...
            subscriptionStatus: Object.entries(subscriptionStatus).map(([status, count]) => ({ status, count })),
            recurrence: (subscriptions ?? []).map((sub: any) => ({ plan: sub.plans?.name ?? "Sem plano", status: sub.status, nextBilling: sub.current_period_end })),
            plans: plans ?? [],
            users: profiles ?? [],
          });
        } catch (error) {
