...
import { BookOpen, CalendarClock, Camera, Check, CreditCard, Heart, Languages, Lock, Mail, Play, Trophy, X } from "lucide-react";
...
import { type AppLanguage, cancelLanguageSwitch, LANGUAGES, setLanguage, useI18n, useLanguageHistory } from "@/lib/i18n";
...
type BillingInfo = {
  email: string | null;
  subscription: { status: string; amount_cents: number; currency: string; current_period_end: string | null; plans?: { name?: string; billing_interval?: string } | { name?: string; billing_interval?: string }[] | null } | null;
  sales: Array<{ id: string; status: string; amount_cents: number; currency: string; sold_at: string; plans?: { name?: string } | { name?: string }[] | null }>;
  paidTotalCents: number;
};

const PROFILE_COPY: Record<AppLanguage, Record<string, string>> = {
  "pt-BR": { account: "Conta", appLanguage: "Idioma do app", emailLocked: "Email bloqueado", emailHint: "O email da compra não pode ser alterado.", financial: "Financeiro", currentPlan: "Seu plano", nextBilling: "Próxima cobrança", noPlan: "Sem plano ativo", paymentHistory: "Histórico de pagamentos", totalPaid: "Total pago", changeOwnPassword: "Trocar sua senha", passwordHint: "Defina uma nova senha para o seu acesso.", newPassword: "Nova senha", confirmPassword: "Confirmar senha", savePassword: "Salvar senha", passwordUpdated: "Senha atualizada", passwordMin: "Use pelo menos 8 caracteres", passwordMismatch: "As senhas não conferem", loadingFinance: "Carregando financeiro...", noPayments: "Nenhum pagamento encontrado.", monthly: "mensal", yearly: "anual", updatingLanguage: "Atualizando histórias…", languageUpdated: "Idioma atualizado — histórias traduzidas", showToastLabel: "Notificar com toast ao trocar o idioma", showToastHint: "Quando desligado, o aviso aparece apenas dentro do seletor.", languageChangedInline: "Atualizado!", cancelSwitch: "Cancelar troca", cancelSwitchHint: "Desfaz a mudança antes que as histórias terminem de recarregar.", switchCanceled: "Troca de idioma cancelada", currentLanguage: "Idioma atual", lastUpdate: "Última atualização", noLanguageHistory: "Nenhuma troca registrada — este é o seu idioma desde sempre.", changedFrom: "de {from}", justNow: "agora mesmo", minutesAgo: "há {n} min", hoursAgo: "há {n} h", daysAgo: "há {n} d", childProfile: "Perfil da criança", childNameLabel: "Nome da criança", childNamePlaceholder: "Digite o nome da criança", childNameHint: "Esse nome será usado nas mensagens do app para deixar a experiência mais especial para a criança.", childNameSaved: "Nome salvo!", childNameRemoved: "Nome removido", childNameError: "Não foi possível salvar o nome", childNameSave: "Salvar", childNameClear: "Limpar", addressStyleLabel: "Como chamar a criança nas mensagens", addressStyleHint: "Você pode mudar a qualquer momento, vale para todas as telas.", addressStyleName: "Pelo nome", addressStyleYou: "Por \"você\"", addressStyleNamePreview: "Boa, Davi! Mais uma página pintada!", addressStyleYouPreview: "Boa! Mais uma página pintada!" },
  "en-US": { account: "Account", appLanguage: "App language", emailLocked: "Locked email", emailHint: "The purchase email cannot be changed.", financial: "Financial", currentPlan: "Your plan", nextBilling: "Next billing", noPlan: "No active plan", paymentHistory: "Payment history", totalPaid: "Total paid", changeOwnPassword: "Change your password", passwordHint: "Set a new password for your access.", newPassword: "New password", confirmPassword: "Confirm password", savePassword: "Save password", passwordUpdated: "Password updated", passwordMin: "Use at least 8 characters", passwordMismatch: "Passwords do not match", loadingFinance: "Loading financial details...", noPayments: "No payment found.", monthly: "monthly", yearly: "yearly", updatingLanguage: "Updating stories…", languageUpdated: "Language updated — stories translated", showToastLabel: "Notify with toast when changing language", showToastHint: "When off, the confirmation shows only inside the selector.", languageChangedInline: "Updated!", cancelSwitch: "Cancel switch", cancelSwitchHint: "Undo the change before stories finish reloading.", switchCanceled: "Language switch canceled", currentLanguage: "Current language", lastUpdate: "Last update", noLanguageHistory: "No change recorded — this has always been your language.", changedFrom: "from {from}", justNow: "just now", minutesAgo: "{n} min ago", hoursAgo: "{n} h ago", daysAgo: "{n} d ago", childProfile: "Child profile", childNameLabel: "Child name", childNamePlaceholder: "Type the child's name", childNameHint: "This name is used across the app to make the experience feel special for the child.", childNameSaved: "Name saved!", childNameRemoved: "Name removed", childNameError: "Couldn't save the name", childNameSave: "Save", childNameClear: "Clear", addressStyleLabel: "How the app addresses the child", addressStyleHint: "You can change this anytime, it applies across the whole app.", addressStyleName: "By name", addressStyleYou: "Use \"you\"", addressStyleNamePreview: "Nice, Davi! Another page colored!", addressStyleYouPreview: "Nice! Another page colored!" },
  "es-ES": { account: "Cuenta", appLanguage: "Idioma de la app", emailLocked: "Email bloqueado", emailHint: "El email de compra no se puede cambiar.", financial: "Financiero", currentPlan: "Tu plan", nextBilling: "Próximo cobro", noPlan: "Sin plan activo", paymentHistory: "Historial de pagos", totalPaid: "Total pagado", changeOwnPassword: "Cambiar tu contraseña", passwordHint: "Define una nueva contraseña para tu acceso.", newPassword: "Nueva contraseña", confirmPassword: "Confirmar contraseña", savePassword: "Guardar contraseña", passwordUpdated: "Contraseña actualizada", passwordMin: "Usa al menos 8 caracteres", passwordMismatch: "Las contraseñas no coinciden", loadingFinance: "Cargando financiero...", noPayments: "No se encontró ningún pago.", monthly: "mensual", yearly: "anual", updatingLanguage: "Actualizando historias…", languageUpdated: "Idioma actualizado — historias traducidas", showToastLabel: "Notificar con toast al cambiar el idioma", showToastHint: "Cuando está apagado, el aviso aparece solo dentro del selector.", languageChangedInline: "¡Actualizado!", cancelSwitch: "Cancelar cambio", cancelSwitchHint: "Deshace el cambio antes de que las historias terminen de recargar.", switchCanceled: "Cambio de idioma cancelado", currentLanguage: "Idioma actual", lastUpdate: "Última actualización", noLanguageHistory: "No hay cambios registrados — este siempre ha sido tu idioma.", changedFrom: "de {from}", justNow: "ahora mismo", minutesAgo: "hace {n} min", hoursAgo: "hace {n} h", daysAgo: "hace {n} d", childProfile: "Perfil del niño", childNameLabel: "Nombre del niño", childNamePlaceholder: "Escribe el nombre del niño", childNameHint: "Este nombre se usa en los mensajes de la app para hacer la experiencia más especial.", childNameSaved: "¡Nombre guardado!", childNameRemoved: "Nombre eliminado", childNameError: "No se pudo guardar el nombre", childNameSave: "Guardar", childNameClear: "Borrar", addressStyleLabel: "Cómo se dirige la app al niño", addressStyleHint: "Puedes cambiarlo cuando quieras, vale para toda la app.", addressStyleName: "Por su nombre", addressStyleYou: "Usar \"tú\"", addressStyleNamePreview: "¡Bien, Davi! ¡Otra página coloreada!", addressStyleYouPreview: "¡Bien! ¡Otra página coloreada!" },
};

const money = (cents = 0, currency = "BRL") => new Intl.NumberFormat("pt-BR", { style: "currency", currency }).format(cents / 100);
...
function formatRelativeTime(timestampMs: number, copy: Record<string, string>, nowMs: number): string {
...
function MiniProfileStat({ label, value, icon }: { label: string; value: string; icon?: React.ReactNode }) {
  return (
    <div className="rounded-xl bg-background/35 p-3 ring-1 ring-inset ring-white/[0.05]">
      ...
      <div className="mt-1 truncate font-display text-sm font-extrabold text-foreground">{value}</div>
    </div>
  );
}
...
function ProfilePage() {
  ...
  const { recentStories, lastEntry, lastStory } = useMemo(() => {
    ...
    const recent = validProgress
      .map((entry) => storyBySlug.get(entry.storySlug)!)
      .slice(0, 12);
  ...
  return (
    <main className="px-5 py-10 sm:px-8 sm:py-14 lg:px-12">
      ...
      <header className="flex items-center gap-5">
        ...
        <div className="min-w-0">
          <h1 className="font-display text-2xl font-extrabold tracking-tight text-foreground sm:text-4xl md:text-5xl">
            ...
            <span className="block truncate">{profileGreeting(language, childName, addressStyle)}</span>
          </h1>
          ...
          >
            ...
            {languageHistory && languageHistory.to === language ? (
              <p className="mt-1.5 text-xs font-medium text-muted-foreground">
                {profileText.lastUpdate}: <span className="font-semibold text-foreground">{formatRelativeTime(languageHistory.at, profileText, nowTick)}</span>
                {languageHistory.from && (
                  <>
                    {" "}·{" "}
                    <span className="tabular-nums">
                      {profileText.changedFrom.replace("{from}", LANGUAGE_SHORT_LABEL[languageHistory.from])}
                    </span>
                  </>
                )}
              </p>
          ...
          <div className="mt-5 grid gap-4">
                      ...
                      onClick={() => {
                        ...
                        if (reverted) {
                          ...
                          if (showLanguageToast) {
                            const restored = (window.localStorage.getItem("ccj.app-language.v1") as AppLanguage | null) ?? "pt-BR";
                            const text = PROFILE_COPY[restored]?.switchCanceled ?? profileText.switchCanceled;
                            toast.success(text);
                          }
                        }
                      }}
            ...
            <div className="flex items-start justify-between gap-3 rounded-xl bg-background/35 p-3 ring-1 ring-inset ring-white/[0.05]">
              <div className="min-w-0">
                ...
                <p className="mt-0.5 text-xs font-medium text-muted-foreground">{profileText.showToastHint}</p>
              </div>
              <Switch
            ...
            <label className="grid gap-2 text-sm font-bold text-foreground">
              ...
              <span className="relative block">
                ...
                <Input value={billing?.email ?? ""} disabled readOnly className="h-11 rounded-xl bg-background/45 pl-10" />
              </span>
              <span className="text-xs font-medium text-muted-foreground">{profileText.emailHint}</span>
            </label>
          </div>
        </div>
        ...
        <div className="rounded-2xl bg-surface/80 p-5 ring-1 ring-inset ring-white/[0.07] shadow-card backdrop-blur sm:p-6">
          ...
          {billingLoading ? (
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-3">
              <MiniProfileStat label={profileText.currentPlan} value={planName(billing?.subscription?.plans) ?? profileText.noPlan} />
              <MiniProfileStat label={profileText.totalPaid} value={money(billing?.paidTotalCents ?? 0, billing?.subscription?.currency ?? "BRL")} />
              <MiniProfileStat label={profileText.nextBilling} value={dateLabel(billing?.subscription?.current_period_end)} icon={<CalendarClock className="h-4 w-4" />} />
            </div>
          )}
          <div className="mt-5 space-y-2">
            <h3 className="font-display text-sm font-bold text-foreground">{profileText.paymentHistory}</h3>
            {(billing?.sales.length ?? 0) === 0 ? (
              <p className="rounded-xl bg-background/35 p-3 text-sm text-muted-foreground">{profileText.noPayments}</p>
            ) : billing?.sales.slice(0, 3).map((sale) => (
              <div key={sale.id} className="flex items-center justify-between gap-3 rounded-xl bg-background/35 p-3 text-sm">
                <span className="min-w-0 truncate text-muted-foreground">{planName(sale.plans) ?? profileText.currentPlan} · {dateLabel(sale.sold_at)}</span>
                <span className="font-bold text-foreground">{money(sale.amount_cents, sale.currency)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <form onSubmit={handlePasswordUpdate} className="mt-6 rounded-2xl bg-surface/80 p-5 ring-1 ring-inset ring-white/[0.07] shadow-card backdrop-blur sm:p-6">
        <p className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.12em] text-primary sm:text-[11px]">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-[1fr_1fr_auto]">
          <Input type="password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} placeholder={profileText.newPassword} autoComplete="new-password" className="h-11 rounded-xl bg-background/45" />
          <Input type="password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} placeholder={profileText.confirmPassword} autoComplete="new-password" className="h-11 rounded-xl bg-background/45" />
          <Button type="submit" disabled={savingPassword} className="h-11 rounded-xl px-5 font-bold">
