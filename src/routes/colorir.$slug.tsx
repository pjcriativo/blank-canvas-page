...
const ACHIEVEMENT_COPY: Record<AppLanguage, Record<string, string>> = {
  "pt-BR": { badge: "História concluída", title: "Parabéns!", levelTitle: "Parabéns! Você desbloqueou uma nova fase!", body: "Você concluiu esta história e avançou na sua jornada. Continue pintando para chegar à próxima conquista.", levelBody: "Ao concluir esta história, seu progresso aumentou e uma nova etapa da sua jornada foi liberada.", progress: "Progresso no perfil", next: "Próxima história", home: "Voltar para histórias", profile: "Ver meu progresso", milestone: "Nova fase" },
  "en-US": { badge: "Story completed", title: "Congratulations!", levelTitle: "Congratulations! You unlocked a new stage!", body: "You completed this story and moved forward in your journey. Keep coloring to reach the next achievement.", levelBody: "By completing this story, your progress increased and a new stage of your journey was unlocked.", progress: "Profile progress", next: "Next story", home: "Back to stories", profile: "See my progress", milestone: "New stage" },
  "es-ES": { badge: "Historia concluida", title: "¡Felicidades!", levelTitle: "¡Felicidades! ¡Desbloqueaste una nueva fase!", body: "Concluiste esta historia y avanzaste en tu jornada. Sigue coloreando para llegar a la próxima conquista.", levelBody: "Al concluir esta historia, tu progreso aumentó y una nueva etapa de tu jornada fue liberada.", progress: "Progreso del perfil", next: "Próxima historia", home: "Volver a historias", profile: "Ver mi progreso", milestone: "Nueva fase" },
};

const PAGE_COMPLETE_COPY: Record<AppLanguage, { badge: string; title: string; body: string; nextPage: string; nextStory: string; close: string; pageOf: string }> = {
  "pt-BR": { badge: "Página concluída", title: "Parabéns! Página completa! 🎉", body: "Você pintou todos os elementos desta página. Continue pintando para concluir a história!", nextPage: "Próxima página", nextStory: "Próxima história", close: "Continuar aqui", pageOf: "Página {page} de {total}" },
  "en-US": { badge: "Page completed", title: "Awesome! Page complete! 🎉", body: "You colored every element on this page. Keep going to finish the story!", nextPage: "Next page", nextStory: "Next story", close: "Stay here", pageOf: "Page {page} of {total}" },
  "es-ES": { badge: "Página concluida", title: "¡Felicidades! ¡Página completa! 🎉", body: "Coloreaste todos los elementos de esta página. ¡Sigue pintando para terminar la historia!", nextPage: "Próxima página", nextStory: "Próxima historia", close: "Seguir aquí", pageOf: "Página {page} de {total}" },
};
...
function ColoringPage() {
  ...
  const coloring = useColoringState({
    ...
    onStoryComplete: (state) => {
      ...
      if (state?.unlockedMilestone) {
        if (
          ...
          toast.success(milestoneToast(language, childName, state.unlockedMilestone, addressStyle), {
            duration: 3500,
          });
          markMilestoneToastShown(state.unlockedMilestone);
        }
      } else if (!hasShownStoryDoneToast(slug) && allowSessionToast(childName, `story-done:${slug}`)) {
    ...
    onPageComplete: ({ pageIndex: completedIndex, done, total }) => {
      ...
      if (done === 1 && !hasShownFirstPageToast(slug) && allowSessionToast(childName, `first-page:${slug}`)) {
  ...
  const nextStory = activeStories.find((item, index) => index > storyIndex && getStoryProgress(item.slug).done === 0) ?? activeStories.find((item) => item.slug !== story.slug && getStoryProgress(item.slug).done === 0) ?? activeStories.find((item) => item.slug !== story.slug);
  ...
  const saveSignature = async () => {
    ...
    if (!parsed.success) {
      ...
      setSignatureError(issueCode === "too_small" ? t("typeName") : issueCode === "too_big" ? t("useUpToLetters") : issueCode === "invalid_string" ? t("lettersOnly") : t("invalidName"));
      return;
    }
    ...
    <main className="fixed inset-0 z-50 flex flex-col bg-cream">
      ...
      {showSignature && (
        ...
        <div className="absolute inset-0 z-20 flex items-end justify-center overflow-y-auto bg-background/45 p-4 pb-32 sm:items-center sm:pb-4">
          ...
          >
            <label className="block font-display text-sm font-bold text-foreground" htmlFor="signature-name">
              {t("signature")}
            </label>
            <input
              id="signature-name"
            ...
            />
            {signatureError && <p className="mt-2 text-sm font-semibold text-destructive">{signatureError}</p>}
          ...
          <section className="achievement-pop relative w-full max-w-md max-h-full overflow-y-auto rounded-3xl border border-primary/25 bg-card text-card-foreground shadow-hero ring-1 ring-white/[0.08]">
            ...
            <div className="bg-gradient-warm px-5 py-6 text-center text-primary-foreground sm:px-8">
              ...
              <p className="mt-2 text-xs font-bold uppercase tracking-[0.16em] text-white/85">
                {pageCompleteCopy.pageOf.replace("{page}", String(pageComplete.pageIndex + 1)).replace("{total}", String(story.pages.length))}
              </p>
            </div>
          ...
          <section className="achievement-pop w-full max-w-lg max-h-full overflow-y-auto rounded-3xl border border-primary/25 bg-card text-card-foreground shadow-hero ring-1 ring-white/[0.08]">
            <div className="bg-gradient-warm px-5 py-6 text-center text-primary-foreground sm:px-8">
              <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-white/20 ring-1 ring-white/35">
                {achievement.unlockedMilestone ? <Trophy className="h-8 w-8" /> : <Award className="h-8 w-8" />}
              </div>
              <p className="mt-4 inline-flex items-center gap-1.5 rounded-full bg-white/18 px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.12em]">
            ...
            <div className="px-5 py-5 sm:px-8 sm:py-6">
              ...
              {achievement.unlockedMilestone && (
                <div className="mx-auto mt-4 flex w-fit items-center gap-2 rounded-full bg-primary/12 px-4 py-2 text-sm font-extrabold text-primary ring-1 ring-primary/25">
                  <Trophy className="h-4 w-4" /> {achievementCopy.milestone}: {achievement.unlockedMilestone}
                </div>
              )}
              <div className="mt-5 rounded-2xl bg-background/45 p-4 ring-1 ring-inset ring-white/[0.06]">
                ...
                <div className="mt-2 h-2 overflow-hidden rounded-full bg-white/[0.08]">
                  <div className="h-full rounded-full bg-gradient-gold transition-all duration-700" style={{ width: `${achievement.overallPct}%` }} />
                </div>
              </div>
              <div className="mt-5 grid gap-2 sm:grid-cols-2">
                {nextStory && (
