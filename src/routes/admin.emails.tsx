  ...
  async function handleSaveSender() {
    ...
    if (!email) {
      toast.error("E-mail principal obrigatório", {
        description:
          "Sem ele, compras aprovadas e recuperação de senha ficam bloqueadas.",
      });
      return;
    }
  ...
  return (
    <div className="space-y-6">
      <header className="rounded-3xl border border-border/60 bg-card/80 p-6 shadow-soft">
        ...
        <h1 className="mt-1 font-display text-3xl font-extrabold text-foreground">Templates de envio</h1>
          ...
          >
            ...
            <div className="flex-1 space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                ...
                <strong className="text-foreground">recuperação de senha</strong> não serão
                enviados aos clientes. Corrija no campo abaixo e salve para liberar os disparos.
              </p>
            </div>
          </div>
        );
        ...
        <div className="mt-4 flex flex-wrap items-center justify-end gap-2">
          <button
            ...
            onClick={() => {
              ...
              const target =
                templates.find((t) => t.enabled) ?? templates[0] ?? null;
              if (!target) {
            ...
            <section className="space-y-4 rounded-3xl border border-border/60 bg-card/80 p-6 shadow-soft">
                    ...
                    >
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-muted-foreground">
                          {blocked ? (
                            <>
                              Defina um <strong className="text-foreground">e-mail principal válido</strong> no
                              card acima para liberar o envio automático de{" "}
                              <strong className="text-foreground">compra aprovada</strong> e{" "}
                              <strong className="text-foreground">recuperação de senha</strong>.
                              Enquanto estiver{" "}
                              {status === "empty" ? "vazio" : "inválido"}, este template{" "}
                              <strong className="text-foreground">não será disparado</strong>.
                            </>
                          ) : (
                            <>
                              Todo disparo deste e-mail usa{" "}
                              <strong className="text-foreground">{sender.sender_email}</strong> como
                              endereço de resposta — sem precisar configurar manualmente.
                            </>
                          )}
                        </p>
                      </div>
              ...
              <div>
                <div className="flex items-center justify-between">
                  <label className="text-sm font-bold text-foreground">Corpo do e-mail (HTML)</label>
        ...
        >
          ...
          >
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Envia o template{" "}
              <strong className="text-foreground">"{active.name}"</strong> para qualquer destinatário
              que você quiser — sem alterar o e-mail principal configurado.
            </p>

            <div className="mt-5">
              <div className="flex items-center justify-between gap-2">
                <label className="text-sm font-bold text-foreground">E-mail de destino</label>
                {sender.sender_email && testEmail.trim().toLowerCase() !== sender.sender_email.trim().toLowerCase() && (
                  <button
                    type="button"
              ...
              />
              <p className="mt-1.5 text-[11px] text-muted-foreground">
                Pode ser qualquer endereço — útil para testar entrega em outras caixas (Gmail, Outlook, etc).
              </p>
            </div>
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <div>
                <label className="text-sm font-bold text-foreground">
                  CC <span className="text-[11px] font-normal text-muted-foreground">(opcional)</span>
                </label>
                <input
                  type="text"
                  value={testCc}
              ...
              <div>
                <label className="text-sm font-bold text-foreground">
                  BCC <span className="text-[11px] font-normal text-muted-foreground">(opcional)</span>
                </label>
                <input
                  type="text"
                  value={testBcc}
            ...
            <div className="mt-4 space-y-1.5 rounded-2xl border border-border/60 bg-background/60 p-3 text-[12px] text-muted-foreground">
              <p className="flex justify-between gap-3">
                ...
                <span className="text-foreground">
                  {sender.sender_name || "Reino das Cores"}
                </span>
              </p>
              <p className="flex justify-between gap-3">
                <span>Reply-to:</span>
            ...
            <div className="mt-3 flex items-start gap-2 rounded-2xl border border-primary/30 bg-primary/5 p-3 text-[11px] text-muted-foreground">
              ...
              <p>
                ...
                — assim você valida o conteúdo na hora. Quando o domínio de envio da plataforma
                for ativado, esse botão passa a disparar o e-mail real automaticamente.
              </p>
            </div>
