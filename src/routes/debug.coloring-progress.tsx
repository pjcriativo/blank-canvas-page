  ...
  const stories: StorySummary[] = useMemo(() => {
    return Object.entries(STORY_PAGES).map(([slug, pages]) => ({
      ...
      pages: pages.map((svg, pageIndex) => {
        ...
        const allFills = Object.fromEntries(
          Array.from(fillable).map((id) => [id, "#A7D89A"]),
        );
        return {
          pageIndex,
    ...
    <main className="mx-auto max-w-5xl px-4 py-6 md:py-10">
      <header className="mb-6">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Para cada página, mostramos quais IDs <code className="rounded bg-muted px-1">fill-*</code>{" "}
          contam para o cálculo de 100% e quais foram excluídos (e por quê). A fonte da verdade é a
          mesma função usada nas miniaturas, no badge &quot;Concluída&quot; e na pintura mágica.
        </p>
        <input
          type="search"
          value={filter}
