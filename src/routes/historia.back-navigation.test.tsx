...
describe("E2E mobile — voltar da história para /home", () => {
  ...
  it("abre /historia/noe-e-a-arca, clica na seta de voltar e retorna para /home", async () => {
    ...
    expect(
      screen.getByRole("heading", { level: 1, name: /Arca de Noé/i }),
    ).toBeTruthy();
    ...
    expect(
      screen.getByText(/Animais, chuva e um lindo arco-íris/i),
    ).toBeTruthy();
    //  • Bloco de texto da descrição (parágrafo principal).
    expect(
      screen.getByText(/Noé construiu uma arca enorme/i),
    ).toBeTruthy();
    ...
    await act(async () => {
      ...
      const dispatchTouch = (type: "touchstart" | "touchend") => {
        const Ctor = (globalThis as unknown as { TouchEvent?: new (t: string, init?: EventInit) => Event }).TouchEvent;
