...
import { AlertTriangle, CheckCircle2, PlugZap, RotateCcw, Send, ShieldAlert, ShieldCheck, ShieldOff, Trash2, XCircle } from "lucide-react";
 ...
 */
function parseValidationIssues(errorMessage: string | null): Array<{ path: string; message: string }> {
...
function AdminIntegrationsPage() {
  ...
  const [webhookTestResult, setWebhookTestResult] = useState<
    | { status: number; ok: boolean; body: string; ranAt: string; usedToken: boolean }
    | null
  >(null);
  ...
  useEffect(() => { setWebhookUrl(`${window.location.origin}/api/perfectpay/webhook`); }, []);
  useEffect(() => { void loadPerfectpayToken(); }, []);
  useEffect(() => { void loadInvalidWebhookEvents(); }, []);
  ...
  async function savePerfectpayToken() {
    ...
    if (error) return toast.error("Não foi possível salvar o token", { description: error.message });
    setPerfectpayTokenSaved(value);
    setPerfectpayTokenSavedAt(value ? ((data?.updated_at as string | null) ?? new Date().toISOString()) : null);
  ...
  async function removePerfectpayToken() {
    ...
    if (error) return toast.error("Não foi possível remover o token", { description: error.message });
    setPerfectpayToken("");
  ...
  async function testPerfectpayWebhook() {
    ...
    const samplePayload = {
      ...
      product: { code: "PPPBTEST", name: "Produto de teste", external_reference: "test-ref", guarantee: 7 },
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-3xl border border-border bg-card/70 p-6 shadow-cinematic backdrop-blur-xl sm:p-7">
        ...
        <h1 className="font-display text-3xl font-extrabold text-foreground sm:text-4xl">Integrações</h1>
        <p className="max-w-2xl text-sm text-muted-foreground">Conecte serviços externos para automatizar vendas, ativações e cancelamentos.</p>
      </section>

      <div className="rounded-3xl border border-border bg-card/85 p-5 shadow-card backdrop-blur-xl">
        <h2 className="font-display text-lg font-extrabold text-foreground">{t("perfectPayIntegration")}</h2>
        <div className="mt-4 space-y-5">
          ...
          {perfectpayTokenSaved ? (
            <div className="flex items-start gap-3 rounded-2xl border border-primary/30 bg-primary/10 p-3">
              ...
              <div className="space-y-0.5 text-xs">
                <p className="font-extrabold text-foreground">Endpoint protegido — Token configurado</p>
                <p className="text-muted-foreground">Apenas requisições com o Public token correto serão aceitas.</p>
                {perfectpayTokenSavedAt ? (
                  <p className="text-[11px] text-muted-foreground">
                    Salvo em{" "}
                    <span className="font-bold text-foreground">
                      {new Date(perfectpayTokenSavedAt).toLocaleString("pt-BR", { dateStyle: "short", timeStyle: "short" })}
                    </span>
                  </p>
                ) : null}
              </div>
            ...
            <div className="flex items-start gap-3 rounded-2xl border border-destructive/40 bg-destructive/10 p-3">
              ...
              <div className="space-y-0.5 text-xs">
                <p className="font-extrabold text-destructive">Endpoint desprotegido — Token não configurado</p>
                <p className="text-muted-foreground">
                  Qualquer pessoa com a URL do webhook pode enviar eventos falsos. Configure o Public token da PerfectPay abaixo para validar a origem.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <span className="flex h-6 w-6 items-center justify-center rounded-full bg-primary/15 text-[11px] font-extrabold text-primary">1</span>
              <p className="text-xs font-bold uppercase tracking-wide text-foreground">URL do Webhook (cole na PerfectPay)</p>
            </div>
            <div className="rounded-xl border border-border bg-background p-3 text-xs text-muted-foreground break-all">{webhookUrl}</div>
            <button
              type="button"
              onClick={() => navigator.clipboard.writeText(webhookUrl).then(() => toast.success("Webhook copiado"))}
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <span className="flex h-6 w-6 items-center justify-center rounded-full bg-primary/15 text-[11px] font-extrabold text-primary">2</span>
              <p className="text-xs font-bold uppercase tracking-wide text-foreground">Token público da PerfectPay</p>
            </div>
            <p className="text-xs text-muted-foreground">
              Na PerfectPay (tela do webhook), copie o campo <strong className="text-foreground">Public token</strong> e cole abaixo. Ele garante que só os webhooks vindos da sua conta PerfectPay sejam aceitos.
            </p>
                    ...
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription asChild>
                        <div className="space-y-3 text-sm">
                          <p>
                            Ao remover o Public token, o endpoint do webhook ficará <strong className="text-foreground">aberto</strong> e
                            passará a aceitar requisições sem nenhuma validação de origem.
                          </p>
                          <div className="rounded-xl border border-destructive/40 bg-destructive/10 p-3 text-xs text-foreground">
                            <p className="mb-1 font-extrabold text-destructive">Impacto desta ação:</p>
                            <ul className="list-disc space-y-1 pl-4 text-muted-foreground">
                              <li>Qualquer pessoa com a URL pode disparar eventos de venda falsos.</li>
                              <li>Compras aprovadas, cancelamentos e reembolsos podem ser forjados.</li>
                              <li>Usuários podem ser ativados ou desativados indevidamente.</li>
                            </ul>
                          </div>
                          <p className="text-xs text-muted-foreground">
                            Use somente se você precisa testar a integração ou trocar o token. Você pode salvar um novo token a qualquer momento.
                          </p>
                        </div>
                      </AlertDialogDescription>
                    </AlertDialogHeader>
            ...
            {perfectpayTokenSaved ? (
              <div className="space-y-1">
                <p className="flex items-center gap-1.5 text-[11px] font-bold text-primary">
                  <CheckCircle2 className="h-3.5 w-3.5" /> Token ativo — webhooks PerfectPay serão validados.
                </p>
                {perfectpayTokenSavedAt ? (
                  <p className="text-[11px] text-muted-foreground">
                    Última atualização:{" "}
              ...
              <p className="flex items-center gap-1.5 text-[11px] font-bold text-destructive">
                <AlertTriangle className="h-3.5 w-3.5" /> Sem token configurado — recomendamos salvar para validar a origem dos webhooks.
              </p>
            )}
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <span className="flex h-6 w-6 items-center justify-center rounded-full bg-primary/15 text-[11px] font-extrabold text-primary">3</span>
              <p className="text-xs font-bold uppercase tracking-wide text-foreground">Testar integração</p>
            </div>
            <p className="text-xs text-muted-foreground">
              Envia um evento de exemplo para a sua URL acima usando o token configurado. O resultado da chamada (status HTTP) aparece abaixo.
            </p>
            <button
              type="button"
            ...
            {webhookTestResult ? (
              ...
              >
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className={`font-display text-sm font-extrabold ${webhookTestResult.ok ? "text-primary" : "text-destructive"}`}>
                    {webhookTestResult.status === 0 ? "Falha de rede" : `HTTP ${webhookTestResult.status}`}
                  </span>
                  ...
                  <span className="text-[11px] text-muted-foreground">
                    {new Date(webhookTestResult.ranAt).toLocaleString("pt-BR", { dateStyle: "short", timeStyle: "short" })}
                  </span>
                </div>
                <p className="text-[11px] text-muted-foreground">
            ...
            <div className="mt-3 space-y-2 rounded-2xl border border-border/60 bg-muted/30 p-3">
              ...
              <p className="text-[11px] text-muted-foreground">
                Busca o último evento real registrado em <span className="font-mono">webhook_events</span> e o reenvia para a sua URL — útil para validar se a checagem retorna 200 ou 400.
              </p>
              <button
                type="button"
              ...
              {webhookReplayResult ? (
                ...
                >
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className={`font-display text-sm font-extrabold ${webhookReplayResult.ok ? "text-primary" : "text-destructive"}`}>
                      {webhookReplayResult.status === 0 ? "Falha de rede" : `HTTP ${webhookReplayResult.status}`}
                    </span>
                    ...
                    <span className="text-[11px] text-muted-foreground">
                      Reenviado {new Date(webhookReplayResult.ranAt).toLocaleString("pt-BR", { dateStyle: "short", timeStyle: "short" })}
                    </span>
                  </div>
                  {webhookReplayResult.sourceReceivedAt ? (
                    <p className="text-[11px] text-muted-foreground">
                      ...
                      <span className="font-bold text-foreground">
                        {new Date(webhookReplayResult.sourceReceivedAt).toLocaleString("pt-BR", { dateStyle: "short", timeStyle: "medium" })}
                      </span>
                    </p>
                  ) : null}
                  <p className="text-[11px] text-muted-foreground">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                <span className="flex h-6 w-6 items-center justify-center rounded-full bg-destructive/15 text-[11px] font-extrabold text-destructive">4</span>
                <p className="text-xs font-bold uppercase tracking-wide text-foreground">Webhooks invalidados</p>
              </div>
              <button
                type="button"
            ...
            <p className="text-xs text-muted-foreground">
              Últimos eventos rejeitados pela validação. Selecione um para ver quais campos faltaram ou vieram inválidos.
            </p>
            ...
            {invalidEvents.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-border bg-muted/20 p-4 text-center text-xs text-muted-foreground">
                {loadingInvalidEvents ? "Carregando..." : "Nenhum webhook invalidado nos últimos eventos."}
              </div>
            ) : (
                        ...
                        >
                          <div className="flex items-center justify-between gap-2">
                            <span className="inline-flex items-center gap-1.5 font-display font-extrabold text-foreground">
                              {isToken ? <ShieldOff className="h-3.5 w-3.5 text-destructive" /> : <AlertTriangle className="h-3.5 w-3.5 text-destructive" />}
                              {isToken ? "Token inválido" : "Payload inválido"}
                            </span>
                            <span className="text-[10px] text-muted-foreground">
                              {new Date(evt.received_at).toLocaleString("pt-BR", { dateStyle: "short", timeStyle: "short" })}
                            </span>
                          </div>
                      ...
                      <div className="space-y-3">
                        <div>
                          <p className="font-display text-sm font-extrabold text-foreground">
                            {isToken ? "Token inválido ou ausente" : "Campos rejeitados pela validação"}
                          </p>
                          <p className="text-[11px] text-muted-foreground">
                            Recebido em{" "}
                            <span className="font-bold text-foreground">
                              {new Date(selected.received_at).toLocaleString("pt-BR", { dateStyle: "short", timeStyle: "medium" })}
                            </span>
                          </p>
                        </div>
                        ...
                        {isToken ? (
                          <div className="rounded-xl border border-destructive/40 bg-destructive/10 p-3 text-xs text-foreground">
                            O webhook foi rejeitado porque o token enviado pela PerfectPay não confere com o token configurado aqui (passo 2). Verifique se o mesmo valor está cadastrado nos dois lados.
                          </div>
                        ) : issues.length === 0 ? (
                          <p className="text-xs text-muted-foreground">
                            Sem detalhamento Zod disponível. Mensagem original: <span className="font-mono">{selected.error_message ?? "—"}</span>
                          </p>
                        ) : (
                          <ul className="divide-y divide-border rounded-xl border border-border">
                            {issues.map((issue, idx) => (
                              <li key={`${issue.path}-${idx}`} className="flex flex-col gap-0.5 p-2.5">
                                <span className="font-mono text-[11px] font-bold text-destructive">{issue.path}</span>
