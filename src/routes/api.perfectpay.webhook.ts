...
const decimalLike = z.union([z.number(), z.string().trim().regex(/^-?\d+(\.\d+)?$/)]).nullable().optional();
    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        if (!parsed.success) {
          ...
          const summary = issues.map((i) => `${i.path}: ${i.message}`).join("; ").slice(0, 500);
          await admin.from("webhook_events").insert({
            provider: "perfectpay",
            event_type: "invalid_payload",
            external_event_id: crypto.randomUUID(),
        ...
        const statusName = SALE_STATUS_MAP[statusKey] ?? data.sale_status_detail ?? `status_${statusKey}`;
        ...
        const currency = data.currency_enum === 1 || data.currency_enum == null ? "BRL" : `ENUM_${data.currency_enum}`;
