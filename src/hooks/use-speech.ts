...
export function useSpeech(text: string, opts: UseSpeechOptions = {}) {
  ...
  useEffect(() => {
    ...
    const pickVoice = () => {
      ...
      const naturalVoiceNames = ["google português", "microsoft maria", "microsoft francisca", "luciana", "português do brasil"];
      const exactVoices = voices.filter((v) => v.lang.toLowerCase() === lang.toLowerCase());
      const natural = exactVoices.find((v) => naturalVoiceNames.some((name) => v.name.toLowerCase().includes(name)));
