...
import { getActiveStories, getProgress, getStoryProgress, hasStoryCompletion, markStoryCompleted, saveProgress } from "@/lib/store";
import {
  catalogPercent,
...
export type UseColoringStateOptions = {
  ...
  onPageComplete?: (payload: { pageIndex: number; storySlug: string; done: number; total: number }) => void;
};
...
async function persistStoryCompletion(storySlug: string, overallPct: number, unlockedMilestone?: string) {
...
export function useColoringState({ story, pageIndex, onStoryComplete, onPageComplete }: UseColoringStateOptions) {
  ...
  const computeFreshStoryProgress = useCallback(
    (currentFills: Record<string, string>) => {
      ...
      if (
        persisted &&
        mergedPagesFills[persisted.pageIndex] === undefined &&
        persisted.fills
      ) {
  ...
  const triggerStoryCompletion = useCallback((freshFills?: Record<string, string>) => {
  ...
  useEffect(() => {
    ...
    if (!isCurrentPageComplete && filledRegions < totalFillRegions && celebratedPageRef.current === pageKey) {
      celebratedPageRef.current = null;
    }
  }, [fills, pageIndex, story.slug, story.pages, totalFillRegions, onPageComplete, computeFreshStoryProgress, triggerStoryCompletion]);
  ...
  }, [computeFreshStoryProgress, onPageComplete, pageIndex, story.pages, story.slug, triggerStoryCompletion]);
