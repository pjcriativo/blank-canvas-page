...
export async function getStoryBySlug(
  ...
  const { story_categories_map: _drop, ...story } =
    data as unknown as StoryRow & { story_categories_map?: unknown };
...
export async function listStoryPages(
  storyId: string,
): Promise<ServiceResult<StoryPageRow[]>> {
  const { data, error } = await supabase
    .from("stories_pages")
    .select("*")
    .eq("story_id", storyId)
