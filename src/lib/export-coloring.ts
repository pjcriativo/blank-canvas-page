...
export async function exportColoringPng(
  ...
  if (!clone.getAttribute("viewBox")) {
    clone.setAttribute(
      "viewBox",
      `0 0 ${clone.clientWidth || 1000} ${clone.clientHeight || 1000}`,
    );
  }
...
function resolveViewBox(svgMarkup: string, pageIndex: number): { x: number; y: number; w: number; h: number } {
