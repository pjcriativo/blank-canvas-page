...
const NUMERIC_ATTR_REGEX = (name: string) => new RegExp(`\\s${name}\\s*=\\s*["'](-?\\d+(?:\\.\\d+)?)`, "i");
...
export function isPageValidationComplete(report: PageValidationReport): boolean {
  ...
  return largeEnoughPage && missingWithinTolerance && report.painted >= report.totalValid - MAX_COMPLETION_TOLERATED_MISSING;
}
