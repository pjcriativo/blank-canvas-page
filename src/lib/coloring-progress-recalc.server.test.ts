...
const storyA: Story = {
  id: "a", slug: "a", title: "A", subtitle: "", shortDescription: "", description: "",
  ageRange: "", testament: "antigo", categoryIds: [], cover: "",
  pages: [{ id: "p1", svg: svgTwoRegions }, { id: "p2", svg: svgTwoRegions }],
  active: true, order: 1,
};
const storyB: Story = {
  ...storyA, id: "b", slug: "b",
...
describe("reconcileServerCompletions — etapa servidor do job", () => {
  it("remove slug do servidor quando local indica < 100%", () => {
    ...
    const local: ColoringProgress[] = [
      { storySlug: "a", pageIndex: 0, fills: {}, completedPages: [0], pagesFills: {}, updatedAt: 1 },
    ];
  ...
  it("PRESERVA slug do servidor quando local confirma 100%", () => {
    const local: ColoringProgress[] = [
      { storySlug: "a", pageIndex: 1, fills: {}, completedPages: [0, 1], pagesFills: {}, updatedAt: 1 },
    ];
    const r = reconcileServerCompletions(local, [storyA], ["a"]);
  ...
  it("NÃO inventa registros — slugs locais 100% que faltam no servidor são ignorados", () => {
    ...
    const local: ColoringProgress[] = [
      { storySlug: "b", pageIndex: 0, fills: {}, completedPages: [0], pagesFills: {}, updatedAt: 1 },
    ];
  ...
  it("ignora índices órfãos em completedPages (i >= total) ao decidir 100%", () => {
    ...
    const local: ColoringProgress[] = [
      { storySlug: "a", pageIndex: 0, fills: {}, completedPages: [0, 5], pagesFills: {}, updatedAt: 1 },
    ];
    const r = reconcileServerCompletions(local, [storyA], ["a"]);
    expect(r.slugsToRemove).toEqual(["a"]);
  });
  ...
  it("processa múltiplos slugs em uma única passada (lote)", () => {
    const local: ColoringProgress[] = [
      { storySlug: "a", pageIndex: 1, fills: {}, completedPages: [0, 1], pagesFills: {}, updatedAt: 1 }, // 100%
