 ...
 */
function makeCoalescer<TPayload>(
  delayMs: number,
  runner: (payload: TPayload) => Promise<void>,
) {
...
const syncStoryProgressDebounced = makeCoalescer<{
  ...
  if (!userId) return;
  const completionPercent = p.totalPages > 0 ? Math.round((p.pagesCompleted / p.totalPages) * 100) : 0;
 ...
 */
export function syncFavoriteToggle(input: {
  storySlug: string;
  ...
  void (async () => {
    ...
    try {
      ...
      const { error } = await supabase
        .from("profiles")
        .upsert(
          { user_id: userId, child_name: sanitized },
          { onConflict: "user_id" },
        );
