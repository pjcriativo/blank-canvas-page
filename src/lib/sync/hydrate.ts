...
async function hydrateFavorites(userId: string): Promise<void> {
  ...
  if (
    ...
    next.some((slug) => !local.includes(slug))
  ) {
...
async function hydrateProgressAndArtworks(userId: string): Promise<void> {
  ...
  const artworkBySlug = new Map<string, Map<number, { fills: Record<string, string>; updatedAt: number }>>();
  ...
  const localBySlug = new Map<string, ColoringProgress>(
    localProgress.map((p) => [p.storySlug, p]),
  );
  ...
  for (const slug of allSlugs) {
    ...
    const pageIndex = dbWins && dbStory
...
export async function hydrateFromSupabase(userId: string): Promise<void> {
  ...
  try {
    await Promise.all([
      ...
      hydrateProgressAndArtworks(userId),
    ]);
    bumpStore();
  } catch (err) {
