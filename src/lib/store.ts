/**
 * Mock store local — persiste em localStorage para simular backend.
 * Pronto para virar Lovable Cloud quando ativarmos auth + banco.
 */
import type { Banner, Category, ColoringPage, ColoringProgress, Story, StoryCompletionRecord } from "./types";
...
export function getStories(): Story[] {
  return load<Story[]>(KEYS.stories, STORIES).map(normalizeStory).map((story) => localizeStory(story, getLanguage()));
}
...
export function getCategories(): Category[] {
  return load<Category[]>(KEYS.categories, CATEGORIES).map((category) => localizeCategory(category, getLanguage()));
}
